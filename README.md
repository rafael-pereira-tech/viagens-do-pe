...
- Local: `cd workers && npm install && npm run dev` — see [`workers/README.md`](workers/README.md) for `SMILES_*` / `TUDOAZUL_LOGIN`+`TUDOAZUL_PASSWORD` / `LATAM_LOGIN`+`LATAM_PASSWORD` placeholders, dry-run fixtures, and a manual `/run` tick
