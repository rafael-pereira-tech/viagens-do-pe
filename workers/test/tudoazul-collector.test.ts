...
describe('tudoazul collector', () => {
  ...
  it('maps 401 to auth_failed', async () => {
    const collector = createTudoAzulCollector(
      { AZUL_ACCESS_TOKEN: 'test-token', AZUL_REQUEST_DELAY_MS: '0' },
      {
  ...
  it('maps HTML 403 WAF to scrape_failed', async () => {
    const collector = createTudoAzulCollector(
      { AZUL_ACCESS_TOKEN: 'test-token', AZUL_REQUEST_DELAY_MS: '0' },
      {
        fetch: async () => new Response(SEARCH_HTML_BLOCK, { status: 403, headers: { 'content-type': 'text/html' } }),
        sleep: async () => {},
      },
    );
    const result = await collector.collect(params);
  ...
  it('returns empty when both searches have no journeys', async () => {
    const collector = createTudoAzulCollector(
      { AZUL_ACCESS_TOKEN: 'test-token', AZUL_REQUEST_DELAY_MS: '0' },
      {
        fetch: async () => jsonResponse(SEARCH_EMPTY),
        sleep: async () => {},
      },
    );
    const result = await collector.collect(params);
  ...
  it('fetches guest token then points and cash availability', async () => {
    const urls: string[] = [];
    const collector = createTudoAzulCollector(
      { AZUL_SUBSCRIPTION_KEY: 'spa-key', AZUL_REQUEST_DELAY_MS: '0' },
      {
        fetch: async (input) => {
          const url = String(input);
          urls.push(url);
          if (url.includes(AZUL_TOKEN_PATH)) {
            return jsonResponse({ data: { token: 'guest-jwt' } });
          }
  ...
  it('treats token failure as auth_failed and does not search', async () => {
    const urls: string[] = [];
    const collector = createTudoAzulCollector(
      { AZUL_SUBSCRIPTION_KEY: 'spa-key', AZUL_REQUEST_DELAY_MS: '0' },
      {
