...
describe('runIngest', () => {
  ...
  it('persists Smiles dry-run PET→CGH snapshots for a one-day window', async () => {
    ...
    const smilesWeb = smiles.filter((row) => row.source === 'smiles_web');
    const voegol = smiles.filter((row) => row.source === 'voegol');
  ...
  it('persists Smiles + TudoAzul + LATAM Pass dry-run snapshots for a one-day window', async () => {
    ...
    assert.ok(smiles.some((row) => row.source === 'smiles_web' && row.miles != null && row.amount_brl == null));
    assert.ok(smiles.some((row) => row.source === 'voegol' && row.amount_brl != null && row.miles == null));
    assert.ok(smiles.filter((row) => row.source === 'smiles_web').every((row) => row.amount_brl == null));
